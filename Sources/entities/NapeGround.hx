...
class NapeGround extends Entity{
	//initialize
	...
	public function getCurvyGroundPoints(){	
		var points = [];	
		for (i in 0...220){
			points.push(new Vec2(i*100,Math.cos(i*.025)*700*Math.sin(i*.1)*Math.sin(i*.01))); 
		}
	...
	public override function render(g:Graphics){
		...
		for (i in 0...points.length-1){
			//trace(points[i]);
			g.drawLine(points[i].x,points[i].y,points[i+1].x,points[i+1].y,2);
		}
